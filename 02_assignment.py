...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    import copy
    more_temperatures=list(more_temperatures)
    temperatures = more_temperatures + temperatures
    temperatures = list(iot_sensor_points.values()) + temperatures
...
def display_menu(menu):
    """R represents the last choice before exiting.  L represents the length of the menu without exit."""
    import time
    l = len(menu)
    ...
    while choice != (l + 1):
        for i in range(0, l):
            menu_number = i + 1
            printout_phrase = str(menu_number) + ". " + menu[i]
            print(printout_phrase)
        print(str(l + 1) + ". Exit")
        print("Please enter your choice as a number and hit enter.")
        choice = input()
        try:
            choice = int(choice)
        except ValueError:
        ...
        if choice == (l + 1):
            break
        def switch_function(choice, r):
            if isinstance(choice, int):
                switch_choices = {1: "Bitcoin was bought.", 2: "Ethereum was bought.", 3: "Bitcoin was sold.",
                                  4: "Ethereum was sold."}
                print(switch_choices.get(choice, 'That is not a valid choice.'))
                time.sleep(1.1)
                r = choice
                return r
        r = switch_function(choice, r)
...
def exercise09():
    ...
    for i in range(0, 10):
        dog_media = r.get(url=url, data=url)
...
def exercise10(sentence):
    ...
    a_string = sentence.swapcase()
    a_string = a_string[::-1]
    reversed = a_string.replace(" ", "_")
