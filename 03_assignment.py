...
class Box:
    ...
    def render(self):
        row = '*' * self.__length +'\n'
        full_box = row * self.__width
        return(full_box)
    def invert(self):
        new_length = self.__width
        new_width = self.__length
    ...
    def __eq__(self,other):
        equality = self.__length == other.__length and self.__width == other.__width
        return(equality)
    def print_dim (self):
        print(self.__length)
        print(self.__width)
    def get_dim(self):
        a_tuple = (self.__length,self.__width)
        return a_tuple
    def combine(self, other):
        ...
        return Box(new_length, new_width)
    def get_hypot(self):
        hypotenuse = math.sqrt(self.__length**2+self.__width**2)
        return hypotenuse
...
class MangoDB:
    def __init__(self):
        ...
        self.__collection['default']['version'] = str(1.0)
        self.__collection['default']['db'] = 'mangodb'
        self.__collection['default']['uuid'] = idvalue
        self.__collections_list = ['default']
    def display_collection(a_library):
        #for key , value in a_library.items():
            #print ('\n' + key)
        for key, value in a_library.items():
            if isinstance(value, dict):
                print('collection:')
                print(key + '\n')
                MangoDB.display_collection(value)
            else:
                print('\t' +key + ":")
                print('\t' + str(value))
                print('')
    def display_all_collections(self):
        print('\t')
        MangoDB.display_collection(self.__collection)
    def add_collection(self,collection_name):
        self.__collection[collection_name] = {}
        self.__collections_list.append(collection_name)
    def update_collection(self,collection_name,updates):
        for key2, value2 in updates.items():
            key_to_add = str(key2)
            value_to_add = value2
        self.__collection[collection_name][key_to_add] = value_to_add
    def remove_collection(self,collection_name):
        if collection_name in self.__collection:
            del self.__collection[collection_name]
            self.__collections_list.remove(collection_name)
    def get_collection_names(self):
        a_list = self.__collections_list
        return(a_list)
    def wipe(self):
        removal_list = self.__collections_list.copy()
        #I added this list so it wouldn't skip while iterating as items are removed from the target,collections_list
        for i in range(0, len(removal_list)):
            MangoDB.remove_collection(self,removal_list[i])
        self.__collections_list = ['default']
        self.__collection['default'] = {}
        idvalue = uuid.uuid4()
        self.__collection['default']['version'] = 1.0
        self.__collection['default']['db'] = 'mangodb'
        self.__collection['default']['uuid'] = idvalue
    def get_collection_size(self,collection_name):
        return( len (self.__collection[collection_name]) )
    def to_json(self, collection_name):
        json_string = str(self.__collection[collection_name])
    ...
    def return_uuid(self):
        print(self.__collection['default'].get('uuid'))
...
'''
    ...
    box2.double()
    box1.combine(box2)
    box_two_size = box2.get_dim()
    for i in range(0, 2):
        print(box_two_size[i])
...
def exercise02():
    ...
    for i in range(0, len(test_scores)):
        a_dictionary = {}
        a_key = i + 1
        a_value = test_scores[i]
        a_dictionary[a_key] = a_value
        db.update_collection('testscores', a_dictionary)
    db.get_collection_size()
    db.list_collections()
    db.return_uuid()
    db.wipe()
    db.return_uuid()
