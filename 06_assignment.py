...
def exercise08():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = pd.read_csv('http://samplecsvs.s3.amazonaws.com/Sacramentorealestatetransactions.csv', skiprows=0)
    row_count = len(df.iloc[:, 10])
    avg_sq_ft = df.mean()[3]
    zip_95670 = df['zip'] == 95670
    df_zip_95670 = df[zip_95670]
    zip_95610 = df['zip'] == 95610
    df_zip_not_95610 = df[-zip_95610]
    # ------ Place code above here /\ /\ /\ ------
...
def exercise09():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = requests.get('https://api.coindesk.com/v1/bpi/historical/close.json?start=2017-09-01&end=2018-10-05', verify= False)
    df = df.json()['bpi']
...
def exercise11(n):
    ...
    rem = 2 * (n % 3) % 3
    array_1d_padded = np.append(array_1d, [0] * rem)
    # The padding allows arrays to be used even if their lengths aren't divisible by 3
    array_reshaped = np.reshape(array_1d_padded, (3, int(len(array_1d_padded) / 3)))
    # ------ Place code above here /\ /\ /\ ------
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.tile(np.identity(2), (n, n))
    # ------ Place code above here /\ /\ /\ ------
...
def exercise13(n):
    ...
    s = pd.Series(np.random.randint(1, n, n), index= index)
    graph_set = s.cumsum()
    background = plt.gca()
    background.axes.get_xaxis().set_visible(False)
    a_plot = graph_set.plot(kind='bar', ax= background)
    plt.show(a_plot)
    # ------ Place code above here /\ /\ /\ ------
    return s
...
def exercise14(words):
    ...
    length_function = lambda k: len(k)
    df = pd.Series(words.map(length_function))
    # ------ Place code above here /\ /\ /\ ------
    return df
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df_again = pd.read_csv('http://samplecsvs.s3.amazonaws.com/Sacramentorealestatetransactions.csv', skiprows=0)

    df = df_again.iloc[list(range(4, len(df_again['street']), 5)), [0, 2]]
    # ------ Place code above here /\ /\ /\ ------
    return df
